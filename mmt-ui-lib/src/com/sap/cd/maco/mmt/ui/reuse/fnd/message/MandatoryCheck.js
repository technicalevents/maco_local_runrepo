sap.ui.define(
  [
    'sap/ui/base/Object',
    'com/sap/cd/maco/mmt/ui/reuse/fnd/message/InputSearch',
  ...
  function(UI5Object, InputSearch, MessageType, bundle, Assert) {
    ...
    return UI5Object.extend('com.sap.cd.maco.mmt.ui.reuse.message.MandatoryCheck', {
      ...
      _checkMandatoryControls: function(aInputControls) {
        ...
        for (var j = 0; j < aInputControls.length; j++) {
          ...
          if (bError) {
            ...
            oControl.setValueStateText(bundle.getText('clientCheckMandatoryCheck'));
            aMessages.push({});
      ...
      _isMandatory: function(oControl) {
        if (oControl.getMandatory) {
          ...
          Assert.ok(oParent, 'input check failed. no parent for control: ' + oControl);
          Assert.ok(oParent.getLabel, 'input check failed. parent of control has no getLabel function: ' + oControl);
          var oLabel = oParent.getLabel();
          Assert.ok(oLabel.getRequired, 'input check failed. label of parent of control has no getRequired function: ' + oControl);
      ...
      _getLabel: function(oControl) {
        if (oControl.getDataProperty) {
          ...
          Assert.ok(oParent.getLabel, 'input check failed. parent of control has no getLabel function: ' + oControl);
          var oLabel = oParent.getLabel();
          Assert.ok(oLabel.getText, 'input check failed. label of parent of control has no getText function: ' + oControl);
