  ...
  function(UI5Object, ODataResponseParser, bundle, startsWith) {
    ...
    return UI5Object.extend('com.sap.cd.maco.mmt.ui.reuse.message.ErrorManager', {
      ...
      _handle: function() {
        ...
        // 4×× = client error
        // 5×× = server error
        var bClientError = startsWith(sStatusCode, '4');
        if (bClientError) {
          if (sStatusCode === '412') {
      ...
      _handleETagError: function() {
        this._showError({
          msg: bundle.getText('messageManagerETagError')
        });
      },

      _handleTimeoutError: function() {
        this._showError({
          msg: bundle.getText('messageManagerTimeoutError')
        });
      },
      ...
      _handleBusinessErrorWithPopover: function(oParser) {
        ...
        var sMsg = this._oInput.businessMessage ? this._oInput.businessMessage : oParser.getErrorMessage();
        if (!sMsg) {
      ...
      _handleTechnicalError: function(oParser) {
        // create params
        var params = {
          msg: bundle.getText('messageTechnicalErrorOccured'),
          details: []
        };
