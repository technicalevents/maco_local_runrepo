sap.ui.define(
  ...
  function(EventProvider, TransactionController, Context, bundle, ODataResponseHandler, Transaction, ODataMetaModelExt) {
    'use strict';

    return EventProvider.extend('com.sap.cd.maco.mmt.ui.reuse.transaction.draft.Transaction', {
      constructor: function(params) {
        ...
        this._oAssert.ok(this._oAppModel, 'cannot init transaction. no app model');
      ...
      whenPropertiesSet: function(params) {
        ...
        /*
        if (!params) {
          invalid.show(new Error('missing params'));
        }
        if (!params.paths) {
          invalid.show(new Error('missing param: paths'));
        }
        if (!params.values) {
          invalid.show(new Error('missing param: values'));
        }
        if (params.paths.length !== params.values.length) {
          invalid.show(new Error('bad params: paths do not have the same length as values'));
        }*/
      ...
      whenDeleted: function(params) {
        ...
        this._oAssert.ok(params.contexts, 'cannot execute transaction whenDeleted. no contexts');
      ...
      whenDraftNewCreated: function(params) {
        // check params
        this._oAssert.ok(params, 'cannot execute transaction whenDraftNewCreated. no oParams');
        this._oAssert.ok(params.path, 'cannot execute transaction whenDraftNewCreated. no path');
      ...
      whenDraftActivated: function(params) {
        // check params
        this._oAssert.ok(params, 'cannot execute transaction whenDraftActivated. no oParams');
        this._oAssert.ok(params.context, 'cannot execute transaction whenDraftActivated. no context');

        // check object
        var oObject = params.context.getObject();
        this._oAssert.ok(!oObject.IsActiveEntity, 'Cannot execute transaction whenDraftActivated for an activate object.');
      ...
      whenDraftEditCreated: function(params) {
        // check params
        this._oAssert.ok(params, 'cannot execute transaction whenDraftEditCreated. no oParams');
        this._oAssert.ok(params.context, 'cannot execute transaction whenDraftEditCreated. no context');

        // check object
        var oObject = params.context.getObject();
        this._oAssert.ok(oObject.HasDraftEntity || oObject.IsActiveEntity, 'Cannot execute transaction whenDraftEditCreated for a create draft.');
      ...
      whenDraftDeleted: function(params) {
        // check params
        this._oAssert.ok(params, 'cannot execute transaction whenDraftDeleted. no params');
        this._oAssert.ok(params.context, 'cannot execute transaction whenDraftDeleted. no context');

        // check object
        var oObject = params.context.getObject();
        this._oAssert.ok(!oObject.IsActiveEntity, 'Cannot execute transaction whenDraftDeleted for an active entity.');
